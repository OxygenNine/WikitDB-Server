        ...
        <>
            ...
            <div className="py-8 max-w-2xl mx-auto">
                <div className="mb-6 border-b border-line pb-4 flex items-center gap-4">
                    ...
                    <h1 className="text-2xl font-bold text-fg">代注册 Wikidot 账号</h1>
                </div>
                {error && (
                ...
                <div className="bg-panel rounded-xl p-6 border border-line space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-fg-2 mb-1">用户名</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-fg-2 mb-1">邮箱</label>
                        <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                            className="w-full bg-sunken border border-line text-fg text-sm rounded-lg focus:ring-accent focus:border-accent-line p-2.5"
                            placeholder="用于接收确认邮件，他人不可见" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-fg-2 mb-1">偏好语言</label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-fg-2 mb-1">确认密码</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-fg-2 mb-1">验证码</label>
                        <div className="flex gap-3 items-center mb-2">
                            {captchaUrl ? (
                                <img src={captchaUrl} alt="captcha" className="h-10 rounded border border-line bg-white px-2" />
                            ) : (
                                <span className="text-fg-3 text-sm">点击按钮加载验证码</span>
                            )}
                            <button onClick={handleGetCaptcha} disabled={loading}
                                className="px-3 py-1.5 bg-raised hover:bg-sunken disabled:opacity-50 text-fg text-xs rounded-lg transition-colors">
                    ...
                    <button onClick={handleRegister} disabled={loading || !name || !email || !password || !captchaAnswer}
                        className="w-full px-5 py-2.5 bg-accent-solid hover:bg-accent-solid-hover disabled:opacity-50 text-accent-fg text-sm font-bold rounded-lg transition-colors">
                        {loading ? '提交中...' : '注册'}
                    </button>
